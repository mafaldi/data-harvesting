...
To identify the correct airport code for scraping, we have
generated a table that combines AENA and Wikipedia data. This
table allows you to search for specific airport codes that match
those used on the AENA website.

While you can search using city names, some cities have multiple
airports, making the three-letter IATA code the most precise
search option.

If you are unsure of the correct code, you can either:

-   Run the **List of Airports** section below to view all
    available airports.

-    Refer to the `Create airport code search table.Rmd` document
    to generate the airport list manually.

To specify your airport, update the `airport_code` variable as a
character string. For example, to select Madrid Adolfo-Suárez
Airport, use:
...
To explore all available airports, continue below. If not,
proceed to "Set your user-agent."
...
We created the list of Airports in the section "List of Airports" at the top of the scraper. We can use it to: 
...
its important to remmener that `scrape_all_airports()` saves
